...
class Crawler:
    ...
    def __init__(self, uid, queue, log_print):
        self._id = uid
        self._queue = queue
    ...
    def get_room_info(self):
        ...
        self.anchor_id = response.json()['data']['uid']
        self._print(r'真实房间号：{}'.format(self.roomid))
        self._print(r'主播uid：{}'.format(self.anchor_id))
...
def main():
    roomid = 4664126
    queue = asyncio.Queue()
    c = Crawler(roomid, queue, print)
    tasks = [asyncio.ensure_future(c.crawl()), asyncio.ensure_future(c.heart_beat_loop())]
    loop = asyncio.get_event_loop()
    try:
        loop.run_until_complete(asyncio.wait(tasks))
    except KeyboardInterrupt as e:
        Crawler.queue4log.put(e)
        Crawler.queue4log.put(asyncio.Task.all_tasks())
        for task in asyncio.Task.all_tasks():
            Crawler.queue4log.put(task.cancel())
        loop.stop()
        loop.run_forever()
    except Exception as e:
        Crawler.queue4log.put(e)
